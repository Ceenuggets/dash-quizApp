...
def select_random_countries(countries_df):
    ...
    n = 0
    while n < 10:
        country = random.choice(countries_df)
...
def provide_random_answers(answer, sample):
    m = 0
    country_options = []
...
def display_country_info(country_names, result, num_answered,df):
    country_rec = pd.DataFrame(df)
    # print(country_rec["Country"].values)
    random_ten = select_random_countries(country_rec["Country"].values)
    if not country_names:
        options = [{"label": option, "value": option} for option in provide_random_answers(random_ten[0], country_rec["Country"].values)]
        num_answered += 1
        ...
        options = [{"label": option, "value": option} for option in provide_random_answers(country_names[0], country_rec["Country"].values)]
